...
def verify_freeze() -> Dict[str, Any]:
    ...
    if FREEZE_BASELINE.exists():
        ...
        lines = [
            ...
            "",
            "This is the first freeze verification run. The current hashes have been saved as the baseline for future comparisons.",
            "",
            "| File | Hash |",
            "|---|---|",
        ]
...
def check_security() -> Dict[str, Any]:
    ...
    if not results["pip"]["available"]:
        lines.append(
            "- **Status:** ⚠️ `pip-audit` not installed — run `pip install pip-audit`"
        )
    elif results["pip"]["ok"]:
        lines.append("- **Status:** ✅ No vulnerabilities found")
    else:
...
def run_tests() -> Dict[str, Any]:
    ...
    lines = [
        ...
        f"- **Status:** {'✅ PASS' if pytest_failed == 0 and pytest_passed > 0 else '❌ FAIL'}",
        "",
        "## JavaScript (npm test)",
        "",
    ]
...
def generate_phase_g_summary(results: Dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append(
        "*This summary is auto-generated by `scripts/hardening/generate_evidence_pack.py`."
    )
    lines.append("")
    return "\n".join(lines)
