...
class TestAPIErrorHandling:
    ...
    def test_run_endpoint_invalid_input_path(self):
        ...
        response = client.post(
            "/api/uar/run",
            json={"goal": "test goal", "input_path": "../../../etc/passwd"},
        )
        assert response.status_code == 400
        data = response.json()
        assert "error" in data["detail"]
    ...
    def test_run_endpoint_invalid_timeout(self):
        ...
        response = client.post(
            "/api/uar/run", json={"goal": "test goal", "timeout_seconds": -1}
        )
        assert response.status_code == 400
        data = response.json()
        assert "error" in data["detail"]
...
class TestRateLimiting:
    ...
    def test_rate_limiting_anonymous(self):
        ...
        for _ in range(15):  # Exceed the limit of 10
            response = client.post(
                "/api/uar/run", json={"goal": "rate limit test"}
            )
    ...
    def test_rate_limiting_authenticated(self):
        ...
        for _ in range(50):  # Should not exceed authenticated limit
            response = client.post(
                "/api/uar/run",
                json={"goal": "auth rate limit test"},
                headers=headers,
            )
            responses.append(response)
    ...
    def test_invalid_api_key(self):
        ...
        response = client.post(
            "/api/uar/run", json={"goal": "test"}, headers=headers
        )
...
class TestSecurityEdgeCases:
    ...
    def test_xss_injection(self):
        ...
        for payload in xss_payloads:
            ...
            assert (
                "dangerous content" in error_detail
    ...
    def test_path_traversal_attempts(self):
        ...
        for attempt in traversal_attempts:
            response = client.post(
                "/api/uar/run", json={"goal": "test", "input_path": attempt}
            )
            assert response.status_code == 400
            data = response.json()
            error_detail = data["detail"]["message"]
            assert (
                "Path traversal" in error_detail
                or "Invalid path" in error_detail
            )
    ...
    def test_large_payload_rejection(self):
        ...
        error_detail = data["detail"]["message"]
        assert (
            "cannot exceed" in error_detail
            or "too long" in error_detail.lower()
        )
...
class TestMemoryAndResourceLimits:
    ...
    def test_event_limiting_in_stream(self):
        ...
        response = client.post(
            "/api/uar/stream",
            json={"goal": "test memory limits", "skills": ["section_sum"]},
        )
...
class TestConcurrentAccess:
    ...
    def test_concurrent_requests(self):
        ...
        def make_request():
            response = client.post(
                "/api/uar/run", json={"goal": "concurrent test"}
            )
