...
def _is_relative_to(path: Path, other: Path) -> bool:
    """Check if path is relative to other, with Python < 3.9 fallback."""
...
def _read_file_safely(file_path: Path, allowed_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        binary_extractors = {
            ...
            ".parquet": (
                "parquet",
                lambda p: _extract_dataframe(p, "parquet"),
            ),
            ".feather": (
                "feather",
                lambda p: _extract_dataframe(p, "feather"),
            ),
        }
        if suffix in binary_extractors:
            ...
            try:
                ...
                logger.exception("%s extraction failed", kind)
                return {
                    "path": str(file_path.relative_to(allowed_root)),
                    "text": "",
                    "size": 0,
                ...
                try:
                    ...
                    if entry_size > MAX_FILE_SIZE:
                        yield {
                            ...
                            if _is_relative_to(entry, allowed_root)
                            else str(entry),
                            "text": "",
                            "size": 0,
        ...
        return {
            ...
            "document_count": len(
                [
                    d
                    for d in documents
                    if "error" not in d or d.get("error") == ""
                ]
            ),
            "paths": [
                doc["path"]
