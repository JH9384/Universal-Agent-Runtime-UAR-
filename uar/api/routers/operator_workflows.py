...
async def get_incident_patterns(
    ...
    for inc in incidents:
        ...
        # Extract words from title for pattern detection
    ...
    # Find recurring words (potential pattern indicators)
    recurring_words = sorted(
        [{"word": w, "count": c} for w, c in title_words.items() if c >= 2],
        key=lambda x: -x["count"],
    )[:10]

    # Runs linked by multiple incidents (hot spots)
    hot_runs = sorted(
        [
    ...
    if recurring_words:
        narrative_parts.append(
            f"Recurring themes: {', '.join(w['word'] for w in recurring_words[:3])}."
        )
    if hot_runs:
        narrative_parts.append(
...
async def get_recommendation_evolution(
    ...
    # Build trust trajectory per type across snapshots
    ...
    # Calculate velocity (change per day) for each type
...
async def get_operator_workflows(
    ...
    if not investigations:
        return {"workflows": [], "narrative": "No investigations to analyze."}

    # Categorize by outcome
    resolved = [inv for inv in investigations if inv.get("status") == "closed"]
    active = [inv for inv in investigations if inv.get("status") == "active"]

    # Action sequence analysis for resolved investigations
    sequence_counts: Dict[str, int] = {}
    for inv in resolved:
        actions = inv.get("actions", [])
        seq = " → ".join(a.get("type", "unknown") for a in actions)
        if seq:
    ...
    if common_sequences:
        narrative_parts.append(
            f"Most common resolution path: {common_sequences[0]['sequence']}."
        )
    if top_actions:
        narrative_parts.append(
...
async def get_knowledge_clusters(
    ...
    # Build co-occurrence matrix: incidents that share runs
    incidents = _load_all_incidents()
    ...
    # Find incident pairs that share runs (clusters)
    pairs: Dict[str, int] = {}
    ...
    # Recommendation categories that frequently co-occur with incidents
    ...
    if clusters:
        narrative_parts.append(f"{len(clusters)} incident cluster(s) found.")
    else:
    ...
    return {
        ...
        "recommendation_category_counts": {
            k: v
            for k, v in sorted(rec_cat_counts.items(), key=lambda x: -x[1])[
                :10
            ]
        },
    }
...
async def get_operator_intelligence(
    ...
    # Actions present in resolved vs unresolved
    ...
    # Lift: how much more frequent in resolved vs unresolved
    ...
    for action, res_count in resolved_actions.items():
        ...
        unres_rate = (
            unres_count / total_unresolved if total_unresolved > 0 else 0
        )
        lift = res_rate / (unres_rate + 0.001)  # smoothing
        action_lift.append(
            {
                "action": action,
    ...
    # Time-to-resolution by first action type
    ...
    for inv in resolved:
        ...
        if actions:
            ...
            if duration > 0:
                time_by_first_action.setdefault(first, []).append(duration)
    ...
    if avg_time:
        narrative_parts.append(
            f"Fastest resolution starts with: {avg_time[0]['first_action']}."
        )
