...
class MetricsCollector:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for endpoint, m in self._endpoint_metrics.items():
                avg_duration = m.total_duration / m.count if m.count > 0 else 0.0
            ...
            return {
                ...
                "error_rate_percent": round(self._total_errors / self._total_requests * 100, 2) if self._total_requests > 0 else 0.0,
    ...
    def get_prometheus_format(self) -> str:
        ...
        with self._lock:
            ...
            lines.append("# HELP uar_request_duration_seconds Request duration")
            lines.append("# TYPE uar_request_duration_seconds histogram")

            for endpoint, m in self._endpoint_metrics.items():
                avg = m.total_duration / m.count if m.count > 0 else 0.0
                lines.append(f'uar_request_duration_seconds{{endpoint="{endpoint}"}} {avg:.4f}')
                lines.append(f'uar_request_count{{endpoint="{endpoint}"}} {m.count}')
                lines.append(f'uar_request_errors{{endpoint="{endpoint}"}} {m.errors}')
